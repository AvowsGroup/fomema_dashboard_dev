class Dashboards::FwInformationController < ApplicationController
  def index
    ...
    if request.format.js? && params[:value].nil?
      ...
      @fw_reg_by_states = State.joins(doctors: :transactions).group('states.name', 'states.long_code').pluck('states.name', 'states.long_code', 'COUNT(*)').map { |name, long_code, count| [name, long_code, count] }
